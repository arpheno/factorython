...
class ProductionLineBuilder:
    ...
    def organize(self, production_line: ProductionLine):
        ...
        ingredient_flows = pd.concat([ingredient_flows, pd.DataFrame(0, index=missing_rows_in_ingredients, columns=ingredient_flows.columns)], axis=0)
        #if some product is missing, we need a new column in the production_flows, that produces 1 of that product
        production_flows = pd.concat([production_flows, pd.DataFrame(0, index=production_flows.index, columns=missing_rows_in_production)], axis=1)
        production_flows.loc[missing_rows_in_production, missing_rows_in_production] = np.eye(len(missing_rows_in_production))
